    ...
    <Sidebar className={`border-r border-teal-200/50 bg-gradient-to-b from-white via-teal-50/20 to-cyan-50/30 dark:border-teal-700/30 dark:from-slate-900 dark:via-slate-800/50 dark:to-slate-900/80 ${collapsed ? "w-16" : "w-64"} transition-all duration-300 ease-in-out shadow-xl backdrop-blur-sm h-screen sticky top-0`} style={{ minHeight: '100vh' }}>
      <SidebarContent className="h-full flex flex-col" style={{ minHeight: '100vh' }}>
        ...
        {!collapsed && !['/login','/register'].includes(currentPath) && authUser && (
          <div className="p-6 border-t border-teal-200/50 bg-gradient-to-r from-teal-50 via-cyan-50/30 to-teal-100/50 dark:border-teal-700/30 dark:from-slate-800/50 dark:via-slate-700/30 dark:to-slate-800/80 relative overflow-hidden mt-auto">
            <div className="absolute inset-0 bg-gradient-to-r from-teal-100/20 to-cyan-100/20 opacity-50 dark:from-teal-900/20 dark:to-cyan-900/20"></div>
            <div className="relative text-sm">
              <p className="font-semibold text-teal-900 dark:text-teal-100">{authUser.fullName}</p>
              <p className="text-teal-700 dark:text-teal-300">{authUser.specialty || 'Doctor'}</p>
              <div className="mt-3 w-12 h-1 bg-gradient-to-r from-teal-500 via-cyan-500 to-teal-600 dark:from-teal-400 dark:via-cyan-400 dark:to-teal-500 rounded-full shadow-sm"></div>
            </div>
          </div>
        )}
      </SidebarContent>
